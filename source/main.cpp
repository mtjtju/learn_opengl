...
int main() {
	...
	Shader shader_blinnphong("shader/blinnphong.vt", "shader/blinnphong.fr");
	Shader shader_blinnphong_light("shader/blinnphong_light_obj.vt", "shader/blinnphong_light_obj.fr");
	...
	while (!glfwWindowShouldClose(window)) // 检查窗口有没有被要求退出
	{
		...
		shader_blinnphong.set_mat4("vInv", vInv);
		shader_blinnphong.set_mat4("pInv", pInv);
		shader_blinnphong.set_float("gridGap", 10);
